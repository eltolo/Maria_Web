# modulo: test_faiss_integration.py
from modulo_db.db_manager import conectar_db, crear_tablas, insertar_documento_y_fragmentos
from modulo_faiss.faiss_index import crear_indice_faiss, buscar_fragmentos
...
def test_faiss_integration():
    ...
    dimension_embedding = 384  # Este es el tamaño del embedding del modelo 'all-MiniLM-L6-v2'
    faiss_index = crear_indice_faiss(dimension_embedding)
    
    # Documento de prueba
    documento_prueba =  """Paso de los Libres es un departamento de la provincia de Corrientes, en el noreste de Argentina, que ocupa 4893.3 km²,4​ en la región sudeste de la provincia. Ocupa el octavo lugar en superficie entre los departamentos de la provincia. Esta superficie representa el 5,3% del total provincial que es de 88 886 km².
...
Departamento Paso de los Libres: Tiene los siguientes límites generales: Norte: Una línea que, iniciándose en el mojón N. O. de la propiedad de Juan Layda, sobre la costa del río Miriñay, en el estero Aguará Cuá, sigue el deslinde Norte de esta propiedad y el de las pertenecientes a Dionisio Roldán y R. Roldán de Vergés, Alejo López Lecube y Juan y José Artola, en el brazo del bañado Guaviravi (Chaco), hasta encontrar el arroyo Guaviravi; Este: El arroyo Guaviravi, desde sus bañados, límite oriental de las propiedades de José y Juan Artola y Luisa Rovira de Rovira, en toda la extensión de su curso, aguas abajo, hasta su desembocadura en el río Uruguay; Sud: El río Uruguay, desde la desembocadura del arroyo Guaviravi, aguas abajo, hasta encontrar la boca del río Miriñay; Oeste: El río Miriñay, desde su desembocadura en el río Uruguay, aguas arriba, hasta el mojón N.O. de la propiedad de Juan Layda, en el estero Aguara Kua."""
    ...
    # Insertar el documento y fragmentarlo
    document_id = insertar_documento_y_fragmentos(conn, "Documento de Prueba", documento_prueba, faiss_index)

    if document_id:
        print(f"Documento de Prueba insertado correctamente con ID {document_id}")

        # Realizar una búsqueda en FAISS
        consulta = "¿Dónde está ubicada Paso de los Libres?"
        ids_encontrados, distancias = buscar_fragmentos(faiss_index, consulta)
        print("ids_encontrados:" ,ids_encontrados, "\ndistancias:", distancias)
        if ids_encontrados is not None:
            print(f"Fragmentos encontrados: {ids_encontrados}")
